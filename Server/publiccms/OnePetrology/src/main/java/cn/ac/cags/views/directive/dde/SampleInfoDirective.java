 ...
 */
@Component
public class SampleInfoDirective extends AbstractTemplateDirective {

    @Override
    public void execute(RenderHandler handler) throws IOException, Exception {
        Integer id = handler.getInteger("id");
        if (CommonUtils.notEmpty(id)) {
            SampleInfo entity = service.getEntity(id);
            if (null != entity) {
            ...
            if (CommonUtils.notEmpty(ids)) {
                List<SampleInfo> entityList = service.getEntitys(ids);
                Map<String, SampleInfo> map = new LinkedHashMap<>();
                for (SampleInfo entity : entityList) {
