...
/**
 * Base exporter for the template and direct output generation. Sets up the
                ...
                if (key.startsWith(ExporterSettings.PREFIX_KEY)) {
                    ...
                    if (key.endsWith(".toolclass")) {
                        try {
                            ...
                            Class<Object> toolClass = ReflectHelper.classForName(value.toString(), this.getClass());
                            Object object = toolClass.getDeclaredConstructor().newInstance();
                            getTemplateHelper().putInContext(
                                    key.substring(ExporterSettings.PREFIX_KEY.length(), key.length() - ".toolclass".length()),
                                    object);
                        } catch (Exception e) {
                            throw new RuntimeException("Exception when instantiating tool " + element.getKey() + " with " + value,
                                    e);
                        }
                    }
                }
