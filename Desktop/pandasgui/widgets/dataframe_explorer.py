...
class DataFrameExplorer(QtWidgets.QWidget):
    def __init__(self, pgdf: PandasGuiDataFrameStore):
        ...
        try:
            self.mapviewer = QMapWidget()
            self.mapviewer.initBaseMap()
            self.mapviewer.setDf(pgdf.df)
        except Exception as ex:
            print(ex)
            myservice.logger.error(ex)
        ...
        self.map_dock = self.add_view(self.mapviewer,"View on map")
