...
class DataFrameViewer(QtWidgets.QWidget):
    def __init__(self, pgdf: PandasGuiDataFrameStore):
        ...
        self.pgdf = pgdf
        if hasattr(pgdf,"k_node"):
              self.k_node = pgdf.k_node  # 复制k_node
        # Local state
        # How to color cells
...
class DataTableModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, parent: DataFrameViewer):
        ...
        if hasattr(parent,"k_node") :
            self.k_node = parent.k_node  # 复制k_node
    ...
    def setData(self, index, value, role=None):
        if role == QtCore.Qt.EditRole:
            ...
            try:
                ...
                if hasattr(self,"k_node"):
                    print("此时更新数据库", self.k_node.name,self.pgdf.df.index[row],row, col, value)
                    myservice.arango.updateOneRecordValue(k_nname=self.k_node.name,field_name=self.pgdf.df.columns[col],key_value=self.pgdf.df.index[row],value=value)
                else:
                    print("此时不更新",  row, col, value)
