...
class PlotTAS(QWidget):
    ui = Ui_TASPlot()

    def __init__(self,parent=None):
        super(PlotTAS, self).__init__(parent)
        self.setupUi()
        self.df = None
    ...
    def doPlot(self):
        ...
        datalist = [{"Name": "Label", "Desc": "点的标签值", "Type": "String","UseDefault":False,"DefaultValue":"a"},
                    ...
                    {"Name": "SiO2", "Desc": "二氧化硅含量", "Type": "Float","UseDefault":False,"DefaultValue":"0"},
                    {"Name": "K2O", "Desc": "氧化钾含量", "Type": "Float","UseDefault":False,"DefaultValue":"0"},
                    {"Name": "Na2O", "Desc": "氧化钠含量", "Type": "Float","UseDefault":False,"DefaultValue":"0"}]
        w=DfColumnMappingWidget(self.df,datalist,parent=self)
        w.showDlg()
        if hasattr(w,"df"):
            df = w.df.reset_index(drop=True)
            taspop = TAS(df=df)
            taspop.setParent(myservice.main_window)
            taspop.TAS()
