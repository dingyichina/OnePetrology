'''
   ...
   本类用来展示知识体系的树形结构，用户自定义的，与当前用户名相同。
   author :dingyi
...
class KTreeWidget4My(QWidget):

    def __init__(self, parent=None):
        super(KTreeWidget4My, self).__init__(parent)
        self.ui = Ui_KTree()
    ...
    def refreshTree(self):
        ...
        self.knode.uuid = myservice.username.upper()  # 默认大写，公共的为ROOT，其它的为登录账号的大写（一般是邮箱）
        # 用线程处理，避免界面卡顿
...
class  WorkerThread(QThread):
    ...
    # 处理提交到db的操作
    def run(self):
        self.statusSignal.emit("My Knowledge Tree building start from："+self.knode.name)
        myservice.logger.info("My Knowledge Tree building start from："+self.knode.name)
        try:
            ...
            self.finishSignal.emit("My Knowledge Tree building  finished:"+self.knode.name)
            myservice.logger.info("My Knowledge Tree building  finished:"+self.knode.name)
        except Exception as ex:
            import traceback
            msg = traceback.format_exc()
            myservice.logger.error(msg)
            self.statusSignal.emit("My Knowledge Tree building Error :"+msg)
            self.finishSignal.emit("My Knowledge Tree building Error :"+msg)

if __name__ == '__main__':
    app = QApplication(sys.argv)
    QApplication.setAttribute(Qt.AA_EnableHighDpiScaling | Qt.AA_DisableWindowContextHelpButton)  # 去掉问号。
    appWin = KTreeWidget4My()
    appWin.show()
