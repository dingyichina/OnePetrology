...
class PlotRaman(QWidget):
    ui = Ui_TASPlot()

    def __init__(self,parent=None):
        super(PlotRaman, self).__init__(parent)
        self.setupUi()
        self.df = None
    ...
    def doPlot(self):
        ...
        datalist = [{"Name": "Label", "Desc": "点的标签值", "Type": "String","UseDefault":False,"DefaultValue":"a"},
                    ...
                    {"Name": "Shift", "Desc": "Shift", "Type": "Float", "UseDefault": False, "DefaultValue": "0"},
                    {"Name": "Strength", "Desc": "Strength", "Type": "Float", "UseDefault": False, "DefaultValue": "0"}]
        w=DfColumnMappingWidget(self.df,datalist,parent=self)
        w.showDlg()
        if hasattr(w,"df"):
            df = w.df.reset_index(drop=True)
            taspop = Raman(df=df,parent=myservice.main_window)

            taspop.Raman()
            taspop.setWindowTitle("TAS (total alkali–silica) diagram Volcanic/Intrusive (Wilson et al. 1989)")
