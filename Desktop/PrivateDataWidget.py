...
from ui.ui_private_data import Ui_PrivateData


class PrivateDataWidget(QWidget):
    ui = Ui_PrivateData()

    def __init__(self, parent=None):
        super(PrivateDataWidget, self).__init__(parent)
        self.setupUi()
        # 初始化combolist
        self.doRefresh()
    ...
    def setupUi(self):
        ...
        self.ui.queryFilterWidget.ui.btnCondition.clicked.connect(self.doCondtionFilter)  #这个由于需要参数，所以在这里设置，有点不太好看，***todo***
    ...
    def doClear(self):
        ...
        for i in range(3):
            if QMessageBox.question(self, 'Question', 'Do you want to clear your data of ' + kname + ' ?  \r\nIt will '
                ...
                return
        # 执行删除，代码暂缺 todo
        sample = {"owner": myservice.username.upper(), "scope": "private"}
        myservice.arango.delete(kname, sample)
        QMessageBox.information(self, "Tips", "Clear private data succeed!")
    ...
    def doRefresh(self):
        ...
        # 初始化combolist
        kn_list = myservice.arango.fetchAllEntity(owner=myservice.username.upper())
    ...
    def buildAQL(self):
        ...
        totalaql = " for c in {}  filter  c.scope =='{}' && c.owner =='{}'  ".format(k_name, 'private',myservice.username.upper())  # 用来查询当前条件下的所有的条数
    ...
    def doFetchData(self):
        # 根据查询参数从数据库中查询数据
        #batchSize = self.ui.spinBatchSize.value()
        ...
        # if not fetchPrivate:
        #     # 只选择public的
        #     sample = {"owner":"ROOT","scope":"public"}
        # else:
        #     sample = {"owner":"ROOT","scope":"public"}
    ...
    def doFinishExtract(self, msg):
        ...
        pgdfs.k_node = self.k_node  # 把知识树节点带进去
...
class WorkerThread(QThread):
    ...
    # 处理提交到db的操作
    def run(self):
        self.statusSignal.emit("Query Private Start:" + self.k_node.name)
        myservice.logger.info("Query Private Start:" + self.k_node.name)
        try:
            ...
            # 去除掉arangodb 的默认列，不在ui上显示，私有数据默认不需要显示owner
            ...
            self.statusSignal.emit("Query private Finished:" + self.k_node.name)
            myservice.logger.info("Query private Finished:" + self.k_node.name)
