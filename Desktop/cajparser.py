        ...
        for i in range(self.page_num):
            ...
            if (text_header_read32[8:20] == b'COMPRESSTEXT'):
                ...
                if (len(output) != expanded_text_size):
                    raise Exception("Unexpected:", len(output), expanded_text_size)
            else:
                caj.seek(page_data_offset)
            ...
            if (images_per_page > 1):
                if (len(page_data.figures) == images_per_page):
                    ...
                    raise Exception("Image Count %d != %d" % (len(page_data.figures), images_per_page))
            ...
            for j in range(images_per_page):
                ...
                if (offset_to_image_data != current_offset + 12):
                    raise Exception("unusual image offset")
                ...
                if (image_type[image_type_enum] == "JBIG"):
                    ...
                    raise Exception("Unknown Image Type %d" % (image_type_enum))
                image_list.append(image_item)
        if (len(image_list) == 0):
            raise Exception("File is pure-text HN; cannot convert to pdf")
