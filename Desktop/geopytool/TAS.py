...
class TAS(AppForm):
    ...
    def create_main_frame(self):
        ...
        self.line_cb.stateChanged.connect(self.TAS)  # int
        ...
        for w in [self.load_data_button, self.save_button, self.result_button, self.save_lda_button_selected,
                  self.save_predict_button_selected, self.slider_left_label, self.slider, self.slider_right_label]:
            self.hbox1.addWidget(w)
            self.hbox1.setAlignment(w, Qt.AlignVCenter)

        for w in [self.legend_cb, self.show_load_data_cb, self.show_data_index_cb, self.shape_cb, self.lda_cb,
                  self.hyperplane_cb, self.line_cb, self.kernel_select_label, self.kernel_select, self.detail_cb,
                  self.irvine_cb]:
    ...
    def Irvine(self, x, a=39.0, b=3.9492, c=-2.1111, d=0.86096, e=-0.15188, f=0.012030, g=-(3.3539 / 10000)):
        ...
        return (a + b * np.power(x, 1) + c * np.power(x, 2) + d * np.power(x, 3) + e * np.power(x, 4) + f * np.power(x,
                                                                                                                     5) + g * np.power(
            x, 6))
        pass
...
if __name__ == '__main__':
    ...
    # 载入df
    df = pd.read_csv("../DataFileSamples/Whole.csv", engine='python')
    taspop = TAS(df=df)
