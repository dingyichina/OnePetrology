...
class EPerson:
    def __repr__(self):
        return (self.id, self.uuid, self.name, self.metadata, self.lastActive,self.canLogIn,self.email, self._links, self.type, self._links)

    def get_full_name(self):
        return self.metadata['eperson.firstname'][0]['value'] + " " + self.metadata['eperson.lastname'][0]['value']
...
class DspaceRestClient:
    ...
    def deleteitem(self,itemuuid):
        ...
        if (rep.status_code == requests.codes.ok):
            ...
            rep.raise_for_status()
        return False
    def getEPersonByEmail(self,email):
        path = "/eperson/epersons/search/byEmail?email="+email
        headers = {'X-XSRF-TOKEN': self.token, 'Authorization': self.authorization,'Accept':'application/hal+json, application/json, */*; q=0.01'}
        rep = self.session.get(self.baseUrl+path, headers=headers, verify=False,
                               cookies=self.session.cookies)
        self.refreshTokenByHeader(rep.headers)
        if (rep.status_code == requests.codes.ok):
...
if __name__ == "__main__":
    ...
    client.login('geowind@126.com', 'petrology')
    client.getEPersonByEmail("geowind@126.com")
    exit( 0 )
