...
class FletMap(Container):
    ...
    def _get_image_cluster(self):
        ...
        smurl = "https://webst01.is.autonavi.com/appmaptile?style=6&x={0}&y={1}&z={2}"
        # smurl = r"https://petrology.deep-time.org/geoserver/gwc/service/tms/1.0.0/dde%3Aworld2021@EPSG%3A3857@png/{0}/{1}/{2}.png"
        ...
        for xtile in range(xmin, xmax+self.screenView[0]):
            ...
            for ytile in range(ymin,  ymax+self.screenView[1]):
                try:
                    print("开始拼装url",self.zoom,xtile,ytile)
                    imgurl = smurl.format(xtile, ytile,self.zoom)
                    print(imgurl)
